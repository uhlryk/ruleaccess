	...
	return function(req, res, next) {
		...
		req.isAllowed = function(resource, params){
			if(resource === undefined || resource === null){
				/**
				 * budując zasób jest brany dokładny route path (string wpisywany w każdym route)
				 * jeśli rout path ma /test/test2/:param
				 * to taką dokładnie strukturę będzie miał resource
				 * jeśli rout ma na końcu / to resource na końcu też ma /
				 * wpisany adres w url nie musi mieć / na końcu bo express dobrze dopasuje url do routera
				 * ale resource w addRule musi być identyczny z router path!
				 */
				resource = req.method + req.baseUrl + req.route.path	;// POST/projects/mode/service
			}
